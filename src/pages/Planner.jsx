...
Rules: xp_reward=30 for easy, 60 medium, 100 hard. Tasks must be specific and actionable. 5-21 days. 2-4 tasks/day.` }]
        })
      })
      ...
      <main className="ml-64 flex-1 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <h1 className="font-syne font-extrabold text-3xl tracking-tight mb-2">Study Planner</h1>
            <p style={{ color:'var(--muted)' }} className="text-sm">Paste your syllabus — AI builds your complete study plan in seconds.</p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-8">
            {[['1','Enter Syllabus'],['2','Review Plan']].map(([num,label],i) => (
              <div key={num} className="flex items-center gap-3">
                ...
                {i===0 && <ChevronRight size={16} style={{ color:'var(--border)' }} />}
              </div>
            ))}
          </div>
          ...
          {step===1 && (
            <div className="space-y-6">
              <div style={{ background:'var(--card)', borderColor:'var(--border)' }} className="border rounded-2xl p-6">
                <label style={{ color:'var(--muted)' }} className="text-xs uppercase tracking-wide mb-3 block font-semibold">📄 Your Syllabus / Exam Schedule</label>
                <textarea value={syllabus} onChange={e=>setSyllabus(e.target.value)} placeholder={EXAMPLE} rows={12}
                ...
                <p style={{ color:'var(--muted)' }} className="text-xs mt-2">Tip: Include subjects, topics, and exam dates for the best results.</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div style={{ background:'var(--card)', borderColor:'var(--border)' }} className="border rounded-2xl p-6">
                  <label style={{ color:'var(--muted)' }} className="text-xs uppercase tracking-wide mb-3 block font-semibold flex items-center gap-2"><Calendar size={13} /> Exam Date (optional)</label>
                ...
                <div style={{ background:'var(--card)', borderColor:'var(--border)' }} className="border rounded-2xl p-6">
                  <label style={{ color:'var(--muted)' }} className="text-xs uppercase tracking-wide mb-3 block font-semibold flex items-center gap-2"><BookOpen size={13} /> Study Hours/Day</label>
              ...
              <button onClick={()=>setSyllabus(EXAMPLE)} style={{ color:'var(--accent)' }} className="text-xs hover:underline flex items-center gap-1">
                <Plus size={12} /> Load example syllabus
              </button>
              ...
              <button onClick={generate} disabled={loading||!syllabus.trim()}
                className="w-full py-4 rounded-2xl text-white font-syne font-bold text-lg hover:-translate-y-0.5 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                style={{ background:'linear-gradient(135deg,var(--accent),#5a48e0)', boxShadow:'0 4px 20px rgba(124,106,255,0.25)' }}>
                {loading ? <><Loader2 size={20} className="animate-spin" /> Generating...</> : <><Sparkles size={20} /> Generate AI Study Plan</>}
              </button>
              {loading && <div style={{ color:'var(--muted)' }} className="text-center text-sm animate-pulse">🤖 Claude is analyzing your syllabus...</div>}
            </div>
          )}
          ...
          {step===2 && plan && (
            <div className="space-y-6">
              <div className="border rounded-2xl p-6" style={{ background:'rgba(124,106,255,0.06)', borderColor:'rgba(124,106,255,0.2)' }}>
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="font-syne font-bold text-xl mb-1">{plan.title}</h2>
                    <p style={{ color:'var(--muted)' }} className="text-sm">{plan.summary}</p>
                  </div>
                  <div className="flex gap-4 text-center ml-6">
                    {[['Days', plan.totalDays, 'var(--accent)'],['Tasks', plan.totalTasks, 'var(--accent2)'],['XP', totalXp.toLocaleString(), 'var(--accent3)']].map(([l,v,c])=>(
                      <div key={l}><p className="font-syne font-extrabold text-2xl" style={{ color:c }}>{v}</p><p style={{ color:'var(--muted)' }} className="text-xs">{l}</p></div>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={()=>{setStep(1);setSaved(false);setPlan(null)}}
                  ...
                  className="flex-1 py-3 rounded-xl border text-sm font-medium transition-all">← Regenerate</button>
                <button onClick={savePlan} disabled={saving||saved}
                  ...
                  {saved ? <><CheckCircle2 size={16} /> Saved!</> : saving ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : <><Plus size={16} /> Save to Dashboard</>}
                </button>
              </div>

              {saved && <p style={{ color:'var(--accent3)' }} className="text-center text-sm animate-fade-up">🎉 All {plan.totalTasks} tasks saved! Go to Dashboard to start.</p>}

              <div className="space-y-4">
                {plan.days.map((day,i) => (
                  <div key={i} style={{ background:'var(--card)', borderColor:'var(--border)' }} className="border rounded-2xl overflow-hidden">
                    <div style={{ background:'var(--surface2)', borderColor:'var(--border)' }} className="flex items-center justify-between px-6 py-4 border-b">
                      <div>
                        <p className="font-syne font-bold text-sm">{day.label}</p>
                        <p style={{ color:'var(--muted)' }} className="text-xs mt-0.5">{new Date(day.date+'T00:00:00').toLocaleDateString('en-IN',{weekday:'long',day:'numeric',month:'short'})}</p>
                      </div>
                    ...
                    <div className="p-4 flex flex-col gap-2">
                      {day.tasks.map((task,j) => (
                        <div key={j} style={{ background:'var(--surface2)' }} className="flex items-center gap-3 p-3 rounded-xl">
                          ...
                          <span className="text-xs font-semibold" style={{ color:'var(--accent3)' }}>+{task.xp_reward} XP</span>
                        </div>
                      ))}
                    </div>
                  </div>
